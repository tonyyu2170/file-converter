...
test("pdf-to-image produces zero off-origin requests during conversion", async ({ page }) => {
  // Drain initial-load requests.
  page.on("request", () => undefined);
  await page.goto("/tools/pdf-to-image", { waitUntil: "networkidle" });
  ...
  // Default range = empty = all pages; no fill needed.
  ...
  expect(
    conversionRequests,
    `pdf-to-image made off-origin requests: ${conversionRequests.join(", ")}`,
  ).toEqual([]);
  expect(
    conversionWebSockets,
    `pdf-to-image opened off-origin WebSockets: ${conversionWebSockets.join(", ")}`,
  ).toEqual([]);
});
