...
// =====================================================================
// Privacy regression for the audio-trim engine.
//
...
test("audio-trim conversion produces zero off-origin requests including ffmpeg-core load", async ({
  page,
}) => {
  const PAGE_PATH = "/tools/audio-trim";
  ...
  expect(
    offOriginRequests,
    `audio-trim made off-origin requests: ${offOriginRequests.join(", ")}`,
  ).toEqual([]);
  expect(
    offOriginWebSockets,
    `audio-trim opened off-origin WebSockets: ${offOriginWebSockets.join(", ")}`,
  ).toEqual([]);
});
