...
test("docx-to-pdf produces zero off-origin requests during conversion", async ({ page }) => {
  // Drain initial-load requests.
  page.on("request", () => undefined);
  await page.goto("/tools/docx-to-pdf", { waitUntil: "networkidle" });
  ...
  expect(
    conversionRequests,
    `docx-to-pdf made off-origin requests: ${conversionRequests.join(", ")}`,
  ).toEqual([]);
  expect(
    conversionWebSockets,
    `docx-to-pdf opened off-origin WebSockets: ${conversionWebSockets.join(", ")}`,
  ).toEqual([]);
});
